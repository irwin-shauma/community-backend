...
public class EventHeaderDao extends AbstractJpaDao<EventHeader> {
	...
	public EventHeader findByTitle(String title) throws Exception {
		...
		sqlBuilder.append("SELECT eh.id, eh.eventHeaderCode, eh.title, eh.eventType FROM event_header eh ")
				.append(" INNER JOIN event_type et ON et.id = eh.eventType ").append(" WHERE eh.title = : title ");

		EventHeader eventHeader = null;
		try {
			Object result = createNativeQuery(sqlBuilder.toString()).setParameter("title", title).getSingleResult();
			if (result != null) {
	...
	public List<EventHeader> findAllByUser(String id, String query, Integer startPage, Integer maxPage)
			throws Exception {
	...
	public List<EventHeader> findAllByType(String eventType, String query, Integer startPage, Integer maxPage)
			throws Exception {
		StringBuilder sql = new StringBuilder()
				.append("SELECT eh.* FROM event_header eh INNER JOIN event_type et ON eh.event_type_id = et.id")
				.append(" INNER JOIN event_detail ed ON ed.event_header_id = eh.id ")
				.append(" WHERE et.\"type\" = :event AND ed.start_date >= now() ORDER BY ed.start_date ASC");
	...
	public Long countAllEvent(String type) throws Exception {
		...
		sqlBuilder.append("SELECT COUNT(eh.id) FROM event_header eh ")
				.append(" INNER JOIN event_type et ON et.id = eh.event_type_id ").append(" WHERE et.type = :type ");
		Long total = 0L;
		try {
